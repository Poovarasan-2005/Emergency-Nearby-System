...
const sessionsDB = new Map<string, EmergencySessionRecord>();
const shareTokensDB = new Map<string, ShareTokenRecord>();
const auditLogsDB: AuditLogRecord[] = [];
...
const logSecurityAudit = (action: string, userId: string, req: Request, details?: any) => {
...
// -------------------------------------------------------------
app.get('/api/nearby-services', async (req: Request, res: Response) => {
  try {
    const lat = parseFloat(req.query.lat as string);
    const lng = parseFloat(req.query.lng as string);
    const radiusKm = Math.min(25, Math.max(1, parseFloat((req.query.radiusKm as string) || '5')));
    const category = (req.query.category as string) || 'all';
    const emergencyCategory = (req.query.emergencyCategory as string) || 'unknown';
...
// Start SOS
app.post('/api/emergency/start', sosLimiter, (req: Request, res: Response) => {
  ...
  const record: EmergencySessionRecord = {
    id: sessionId,
    userId: userId || 'anonymous',
...
// Cancel SOS
app.post('/api/emergency/cancel', (req: Request, res: Response) => {
...
// Get Session History
app.get('/api/emergency/history', (req: Request, res: Response) => {
  const userId = req.query.userId as string;
  if (!userId) return res.status(400).json({ error: 'userId is required' });
...
app.post('/api/emergency/share', (req: Request, res: Response) => {
  ...
  const shareRecord: ShareTokenRecord = {
    token,
...
app.get('/api/emergency/share/:token', (req: Request, res: Response) => {
...
// Revoke share token
app.delete('/api/emergency/share/:token', (req: Request, res: Response) => {
  const { token } = req.params;
  const record = shareTokensDB.get(token);
...
// -------------------------------------------------------------
app.post('/api/emergency/contacts/test-alert', (req: Request, res: Response) => {
  ...
  const dispatchId = `SMS-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
  res.json({
    success: true,
    dispatchId,
